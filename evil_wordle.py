...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        post: None
        """
        for i, feedback in enumerate(feedback_colors):
            if feedback == CORRECT_COLOR:
                self.colors[guessed_word[i]] = CORRECT_COLOR
            elif feedback == WRONG_SPOT_COLOR and self.colors[guessed_word[i]] != CORRECT_COLOR:
                self.colors[guessed_word[i]] = WRONG_SPOT_COLOR
            elif (feedback == NOT_IN_WORD_COLOR and self.colors[guessed_word[i]] != WRONG_SPOT_COLOR
                   and self.colors[guessed_word[i]] != CORRECT_COLOR):
    ...
    def __str__(self):
        ...
        for i, row in enumerate(self.rows):
            ...
            for letter in row:
                final += f"{color_word(self.colors[letter], letter)} "
            final = final[:-1]
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for letter in secret_word:
        if letter in secret_word_counts:
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            ...
            secret_word_counts[guessed_word[i]] -= 1

    for i in range(NUM_LETTERS):
        if feedback[i]:
            continue
        if guessed_word[i] in secret_word_counts and secret_word_counts[guessed_word[i]] > 0:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        ...
        if feedback not in word_groups:
            word_groups[feedback] = []
        word_groups[feedback].append(word)

    for group, words in word_groups.items():
        group = WordFamily(group, words)
